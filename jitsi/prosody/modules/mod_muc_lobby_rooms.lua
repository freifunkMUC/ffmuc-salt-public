...
function attach_lobby_room(room)
    local node = room_jid_match_rewrite(room.jid);
    local lobby_room_jid = node .. '@' .. lobby_muc_component_config;
    module:log("error","krombel gonna create lobby room jid = %s created", lobby_room_jid);
    if not lobby_muc_service.get_room_from_jid(lobby_room_jid) then
        ...
        module:log("error","Lobby room jid = %s created",lobby_room_jid);
